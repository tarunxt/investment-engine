...
primary_celery_worker_effective_queues() {
  # Stage 3 execution/reconciliation must not compete with email or planning.
  # Keep this primary worker dedicated to ai even when a legacy environment
  # still contains CELERY_WORKER_QUEUES=ai,email.

  local configured_queues="$1"
  if [[ ",${configured_queues//[[:space:]]/}," != *",ai,"* ]]; then
    printf '%s\n' \
  ...
  if [[ ",${configured_queues//[[:space:]]/}," == *",email,"* ]]; then
    printf '%s\n' \
      'INFO: email is isolated on the dedicated email worker; the ai worker will not consume it.' \
      >&2
  fi
  printf 'ai\n'
}
