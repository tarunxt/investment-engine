...
Stage 1 v5 captures the normalized source universe through Gamma market-keyset pagination before any inherited 007 hard filters and records an explicit completeness proof. Its five-minute full-universe budget is isolated to 008; 007 retains the shared 90-second default. Stage 2 v2 retains the raw provider payload even when strict JSON parsing fails. Stage 3 v3 cannot pass without both cluster IDs, a shared driver, a main joint-loss trigger, and an adjudication reason for every market. Stage 4 v2 emits one target row per analysed market and calculates weighted metrics over the full current-plus-proposed target exposure. These are new versioned 008 facts only; earlier 008 and all 007 records remain frozen.
