...
- Bullpen 008 opts into ordered Gamma `/events` pagination with a rolling deadline cursor so the source universe is not truncated at the legacy offset cap. Only events sharing the current deadline use an offset, and a non-advancing cursor fails closed; Bullpen 007 keeps the legacy scanner pagination and 90-second defaults unchanged.
