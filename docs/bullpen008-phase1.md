...
- Bullpen 008 opts into Gamma's cursor-based `/events/keyset` pagination so the source universe is not truncated at the legacy offset cap. Five-hundred-event pages minimize cursor round trips; Bullpen 007 keeps the legacy scanner pagination and 90-second defaults unchanged.
