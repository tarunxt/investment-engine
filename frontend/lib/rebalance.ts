...
function formatPortfolioEvents(analysis: EventsAnalysis) {
  if (!analysis) return '_No latest Events report available._';
  ...
  return [
    ...
    table?.raw_markdown || '_Events report has no parsed table output yet._',
  ].join('\n\n');
}
...
## 4. Latest Events Report
${formatPortfolioEvents(events)}`;
}
