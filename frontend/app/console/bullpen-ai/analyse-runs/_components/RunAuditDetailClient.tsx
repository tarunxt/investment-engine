        ...
        <SectionShell title="Stage 1 Summary">
          <DetailGrid
            items={[
              ...
              { label: "Scan Scope", value: stringValue(scanContext?.scan_scope) },
              { label: "Scan Completeness", value: stringValue(scanContext?.scan_completeness) },
              { label: "Bullpen Trending Rows", value: String(scanContext?.bullpen_trending_rows ?? "—") },
              { label: "Complete Catalogue Markets", value: String(scanContext?.complete_catalogue_markets ?? "—") },
              { label: "Active Wallet Positions", value: String(scanContext?.active_wallet_positions ?? "—") },
              { label: "Filter-Eligible Markets", value: String(scanContext?.filter_eligible_markets ?? "—") },
              { label: "Wallet Markets Added to Union", value: String(scanContext?.active_wallet_markets_added_to_union ?? "—") },
              { label: "Missing Wallet Markets", value: String(scanContext?.missing_active_market_count ?? "—") },
