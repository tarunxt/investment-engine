...
export function BullpenRunHistoryContent({ page, trends, loading, trendsLoading, error, trendsError, detailLoadingId, onRefresh, onPage, onOpenRun, onClose, showFullScreen = true, eyebrow = "Run History", title = "Bullpen Auto and Manual Runs", fullScreenPath = "/console/bullpen-ai/history", loadReturnsFormula, saveReturnsFormula }: BullpenRunHistoryContentProps) {
  ...
  return <div className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-[0_32px_90px_-32px_rgba(15,23,42,.45)]">
    ...
    </div>{showClusterJson && <BullpenClusterJsonDialog rows={clusterState.rows} marketIds={new Set(trends?.events.map(event => event.market_id) ?? [])} onApply={clusterState.save} onClose={() => setShowClusterJson(false)} />}{scoreEvent && <ScoreCalculation event={scoreEvent} onClose={() => setScoreEvent(null)} />}{llmQuestion && <BullpenLlmBreakdownDialog question={llmQuestion} onClose={() => setLlmQuestion(null)} />}{returnsQuestion && <BullpenInvestmentMathDialog focus="returnsPerDay" question={returnsQuestion} onClose={() => setReturnsQuestion(null)} />}{showReturnsFormula && <BullpenReturnsPerDayFormulaDialog loadFormula={loadReturnsFormula} saveFormula={saveReturnsFormula} onClose={() => setShowReturnsFormula(false)} />}
  </div>;
}
