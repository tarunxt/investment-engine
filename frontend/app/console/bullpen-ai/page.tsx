...
const TABS: {
...
}[] = [
  {
    ...
    description: "All Bullpen Yes/No questions closing in under 30 days.",
  },
  {
    mode: "end-of-month",
    label: "End of Month",
    ...
    description:
      "All Bullpen Yes/No questions ending exactly on June 30, 2026.",
  },
];
...
export default function BullpenAiPage() {
  ...
  const filteredResult = useMemo(
    () => (result?.mode === activeMode ? result : null),
    [activeMode, result],
  );
  ...
  async function runScan() {
    ...
    setError(null);
    try {
      const response = await fetch(`/api/bullpen-ai?mode=${activeMode}`, {
        cache: "no-store",
      });
      const payload = (await response.json()) as ScanResult;
      setResult(payload);
      if (!response.ok || payload.error)
        setError(payload.error || "Bullpen scan failed.");
      else if (payload.warning) setError(payload.warning);
      else if (payload.questions.length === 0) {
        setError(
          "No Bullpen questions matched the current scan filters. Rerun the scan later.",
        );
      }
    } catch (scanError) {
      setError(
        scanError instanceof Error ? scanError.message : "Bullpen scan failed.",
      );
    } finally {
      setIsScanning(false);
    }
    ...
    <div className="mx-auto flex w-full max-w-7xl flex-col gap-6 p-6">
      <div>
        ...
        <p className="mt-2 max-w-3xl text-sm text-slate-600">
          Run Bullpen scans for all binary Yes/No prediction questions in the
        ...
        <CardContent className="space-y-5">
          <div className="flex flex-wrap items-center gap-4">
            <p className="max-w-2xl text-sm text-slate-600">
              The scan no longer applies a question limit; it returns every
              Bullpen question available from the selected source and time
              window.
            </p>
            <Button
              onClick={() => runScan()}
            ...
            <a
              href={
                filteredResult?.sourceUrl ||
                (activeMode === "end-of-month"
              ...
              target="_blank"
              rel="noreferrer"
              className="inline-flex items-center gap-1 whitespace-nowrap text-sm font-medium text-purple-700 hover:text-purple-900"
            >
          ...
          {error ? (
            <div className="flex gap-3 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">
              ...
              <p>
                {error}. If Bullpen blocks server-side access, rerun after
                network access/session availability is restored.
              </p>
            </div>
          ) : null}
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-slate-200 text-sm">
                ...
                <tbody className="divide-y divide-slate-100 bg-white">
                  {filteredResult?.questions.length ? (
                    filteredResult.questions.map((question) => (
                    ...
                    <tr>
                      ...
                      >
                        {isScanning
                          ? "Scanning Bullpen..."
                          : "No scan results yet. Click Run Bullpen Scan to load every available question."}
                      </td>
                    </tr>
                  )}
                </tbody>
