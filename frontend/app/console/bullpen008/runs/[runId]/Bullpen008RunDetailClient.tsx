...
import type { Bullpen008Run } from "@/types/api";
...
export function Bullpen008RunDetailClient({ runId }: { runId: string }) {
  ...
  const activeStage = run.stages.find((stage) => stage.stage_number === selectedStage);
  ...
  return (
    <div className="mx-auto w-full max-w-7xl space-y-6 p-6">
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        {run.stages.map((stage) => <button key={stage.stage_number} type="button" onClick={() => setSelectedStage(stage.stage_number)} className="rounded-3xl border border-slate-200 bg-white p-5 text-left shadow-sm hover:border-sky-300 hover:shadow-md"><p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Stage {stage.stage_number}</p><div className="mt-2 flex items-center justify-between gap-3"><h2 className="font-semibold text-slate-950">{stage.stage_name}</h2><span className="rounded-full border border-slate-200 bg-slate-50 px-2 py-1 text-xs font-semibold capitalize">{stage.status}</span></div><p className="mt-3 line-clamp-2 text-sm text-slate-600">{stage.pass_condition}</p><p className="mt-3 text-xs text-slate-500">{stage.duration_seconds.toFixed(2)}s · {stage.stage_version}</p></button>)}
      </div>
      {activeStage ? <BullpenAutoRunStageOutputDialog stageTitle={`Stage ${activeStage.stage_number}: ${activeStage.stage_name}`} stageDetail={`${activeStage.status.toUpperCase()} · Pass condition: ${activeStage.pass_condition}`} eyebrow="Bullpen 008 immutable stage record" outputLabel="Stage record" recordPageSize={25} deferRawJson outputs={{ inputs: activeStage.inputs, calculations: activeStage.calculations, outputs: activeStage.outputs, rejections: activeStage.rejections, warnings: activeStage.warnings, provenance: activeStage.provenance, start_time: activeStage.started_at, end_time: activeStage.completed_at, duration_seconds: activeStage.duration_seconds, stage_version: activeStage.stage_version, output_hash: activeStage.output_hash }} onClose={() => setSelectedStage(null)} /> : null}
    </div>
  );
}
