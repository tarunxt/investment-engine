...
export type DashboardThreatAnalysis = ZerodhaThreatAnalysis | IndMoneyUsThreatAnalysis;
export type DashboardThreatTableSection = ZerodhaThreatTableSection | IndMoneyUsThreatTableSection;
...
export function countThreatSeverities(tables: DashboardThreatTableSection[] | null | undefined): DashboardSeverityCounts {
...
export function getThreatSummaryPoints(analysis: DashboardThreatAnalysis | null | undefined) {
  const summary = analysis?.report?.summary;
  if (!summary) {
    return [];
  }

  return SUMMARY_POINT_CONFIG
    .map<DashboardSummaryPoint | null>((item) => {
...
export function sortUrgentActionRows(rows: DashboardUrgentActionRow[]) {
  return [...rows].sort((left, right) => {
    ...
    const deadlineDelta = extractDateTimestamp(left.deadline) - extractDateTimestamp(right.deadline);
    if (deadlineDelta !== 0) {
