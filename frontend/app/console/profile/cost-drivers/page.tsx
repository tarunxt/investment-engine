...
type Dashboard = { summary: Record<string, unknown>; dailyCostTrend: MetricRow[]; dataTransferTrend: MetricRow[]; topServices: MetricRow[]; topUsageTypes: MetricRow[]; costDrivers: CostDriver[]; traffic: Traffic[]; recommendations: Recommendation[]; inventory: Record<string, unknown>; debug: Record<string, unknown>; diagnostics?: Diagnostic[] };
...
export default function PlatformCostDriversPage() {
  ...
  return <div className="w-full max-w-7xl space-y-6">
    ...
    {data.debug.mockMode === true && <div className="rounded-md border border-purple-200 bg-purple-50 p-3 text-sm font-medium text-purple-800">Demo data — not real AWS account findings. Placeholder media/image routes are intentionally excluded so nonexistent assets are not shown as cost drivers.</div>}
    {data.debug.mockMode !== true && data.costDrivers.length === 0 && <div className="rounded-md border border-amber-200 bg-amber-50 p-3 text-sm text-amber-800"><b>Live AWS data was not returned.</b> Open diagnostics below to see whether AWS credentials, IAM permissions, Cost Explorer, or stored traffic rollups are missing.</div>}

    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">{summaryCards.map(([label, value, hint]) => <Card key={String(label)} size="sm"><CardHeader className="pb-0"><CardTitle className="font-sans text-xs font-semibold normal-case tracking-normal text-muted-foreground">{label}</CardTitle></CardHeader><CardContent><div className="text-2xl font-semibold tracking-tight">{value}</div><p className="mt-1 text-xs text-muted-foreground">{hint}</p></CardContent></Card>)}</div>

    <div className="grid gap-4 xl:grid-cols-3"><Card size="sm"><CardHeader><CardTitle className="font-sans text-base normal-case tracking-normal">Daily AWS cost trend</CardTitle></CardHeader><CardContent><MiniBars rows={data.dailyCostTrend.slice(-14)} valueKey="cost" labelKey="date" /></CardContent></Card><Card size="sm"><CardHeader><CardTitle className="font-sans text-base normal-case tracking-normal">Data transfer vs 100 GB</CardTitle></CardHeader><CardContent><MiniBars rows={data.dataTransferTrend.slice(-14)} valueKey="gb" labelKey="date" /></CardContent></Card><Card size="sm"><CardHeader><CardTitle className="font-sans text-base normal-case tracking-normal">Top AWS services</CardTitle></CardHeader><CardContent><MiniBars rows={data.topServices} /></CardContent></Card></div>

    <Card size="sm"><CardHeader><CardTitle className="font-sans text-base normal-case tracking-normal">Top cost drivers</CardTitle></CardHeader><CardContent className="overflow-x-auto"><table className="w-full min-w-[1100px] text-left text-xs"><thead className="border-b text-muted-foreground"><tr>{["#","Driver","Source","MTD","Projected","Usage","Confidence","Severity","Why it costs money","Suggested action","Savings"].map(h => <th key={h} className="p-2">{h}</th>)}</tr></thead><tbody>{data.costDrivers.length ? data.costDrivers.map(d => <tr key={d.rank} className="border-b align-top"><td className="p-2">{d.rank}</td><td className="p-2 font-medium">{d.driver}</td><td className="p-2">{d.source}</td><td className="p-2">{money(d.monthToDateCost)}</td><td className="p-2">{money(d.projectedMonthEndCost)}</td><td className="p-2">{d.usageQuantity} {d.unit}</td><td className="p-2">{d.confidence}</td><td className="p-2"><span className={`rounded px-2 py-1 ${severityClass(d.severity)}`}>{d.severity}</span></td><td className="p-2">{d.whyItCostsMoney}</td><td className="p-2">{d.suggestedAction}</td><td className="p-2">{money(d.estimatedMonthlySavings)}</td></tr>) : <tr><td colSpan={11} className="p-4 text-center text-sm text-muted-foreground">No AWS Cost Explorer service spend is available from the latest live check. Review diagnostics and AWS IAM/Cost Explorer setup.</td></tr>}</tbody></table></CardContent></Card>

    <Card size="sm"><CardHeader><CardTitle className="font-sans text-base normal-case tracking-normal">Top bandwidth routes/assets: is it images, videos, API, JS, HTML, or bots?</CardTitle></CardHeader><CardContent className="overflow-x-auto"><table className="w-full min-w-[1000px] text-left text-xs"><thead className="border-b text-muted-foreground"><tr>{["Path","Type","Requests","Total GB","Transfer cost","Cache hit","Top UA","Recommendation"].map(h => <th key={h} className="p-2">{h}</th>)}</tr></thead><tbody>{data.traffic.length ? data.traffic.map(t => <tr key={t.path} className="border-b align-top"><td className="max-w-[260px] truncate p-2 font-medium">{t.path}</td><td className="p-2">{t.classification}<br/><span className="text-muted-foreground">{t.contentType}</span></td><td className="p-2">{t.requests.toLocaleString()}</td><td className="p-2">{t.totalGB}</td><td className="p-2">{money(t.estimatedTransferCost)}</td><td className="p-2">{Math.round(t.cacheHitRate * 100)}%</td><td className="p-2">{t.topUserAgent}</td><td className="p-2">{t.recommendation}</td></tr>) : <tr><td colSpan={8} className="p-4 text-center text-sm text-muted-foreground">No app traffic attribution is available from the latest live checks.</td></tr>}</tbody></table></CardContent></Card>

    <div className="grid gap-4 xl:grid-cols-[minmax(0,2fr)_minmax(320px,1fr)]">
      <Card size="sm">
        <CardHeader><CardTitle className="font-sans text-base normal-case tracking-normal">Rule-based recommendations</CardTitle></CardHeader>
      ...
      <Card size="sm">
        <CardHeader><CardTitle className="flex items-center gap-2 font-sans text-base normal-case tracking-normal">Checks and permissions <ChevronDown className="h-4 w-4" /></CardTitle></CardHeader>
        <CardContent className="space-y-2 text-sm">
          <details>
            ...
            <div className="mt-3 space-y-2">
              ...
              {(data.diagnostics || (data.debug.diagnostics as Diagnostic[] | undefined) || []).map((d) => <p key={`${d.service}-${d.status}`}><b>{d.service}:</b> {d.status} — {d.message}</p>)}
            </div>
          </details>
        </CardContent>
      </Card>
