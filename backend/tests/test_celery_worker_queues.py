...
def test_primary_worker_adds_ai_when_config_omits_it_and_preserves_extra_queues():
    result = _effective_queues(" email, auto_live , custom ")

    assert result.returncode == 0, result.stderr
    assert result.stdout.strip() == "email,auto_live,custom,ai"
    assert "WARNING: CELERY_WORKER_QUEUES omitted mandatory queue ai" in result.stderr


def test_primary_worker_normalizes_whitespace_without_warning_when_ai_is_present():
    result = _effective_queues(" ai, email , custom ")

    assert result.returncode == 0, result.stderr
    assert result.stdout.strip() == "ai,email,custom"
