...
from app.domains.polymarket_direct.providers import (
    DirectPolymarketReadOnlyProvider,
    MarketDataProvider,
    MockProvider,
    aggregate_traders,
...
class PolymarketPaperCopyBot:
    def __init__(
        ...
        self.doctor_status = PolymarketDoctorStatus(
            ok=False, message="Direct Polymarket doctor has not run."
        )
    ...
    async def debug_discovery(self, target: str) -> object:
        provider = (
            self.provider
            if isinstance(self.provider, DirectPolymarketReadOnlyProvider)
            else None
        )
        if not provider:
            raise RuntimeError(
    ...
    async def _read_top_traders_unlocked(self) -> list[PolymarketTrader]:
        try:
            ...
            if self.active_mode == "live-trading" or self._wants_live_execution():
                self._add_activity(f"Direct Polymarket live-read traders failed: {message}")
                await self.logger.warn(f"Direct Polymarket live-read traders failed: {message}")
                raise
            await self.logger.warn(
    ...
    async def _read_recent_trades_unlocked(
        ...
        try:
            ...
            self.live_source_status.last_live_read_error = message
            if self.active_mode == "live-trading" or self._wants_live_execution():
                self._add_activity(f"Direct Polymarket live-read trades failed: {message}")
                await self.logger.warn(f"Direct Polymarket live-read trades failed: {message}")
                raise
            await self.logger.warn(
    ...
    def _startup_block_message(self, block_reason: str) -> str:
        if block_reason != "Direct Polymarket doctor must pass.":
            return block_reason
    ...
    async def _redeem_and_claim_completed_positions(self) -> bool:
        ...
        async with self._redeem_claim_lock:
            ...
            except DirectPolymarketCommandError as exc:
                message = redact_secrets(str(exc))
                if not is_redeem_metadata_lookup_warning(message):
                    raise
                had_redeem_metadata_warning = True
                await self.logger.warn(
                    "Direct Polymarket redeem skipped a resolved market missing Gamma metadata: "
                    f"{message}"
                )
                self._add_activity(
                    "Direct Polymarket redeem checked resolved positions but skipped a market missing Gamma metadata."
                )
            ...
            try:
                ...
                if is_claim_command_unavailable_warning(message):
                    await self.logger.warn(
                        "Direct Polymarket claim command is unavailable in this runtime; redeem completed: "
                        f"{message}"
                    )
                    return had_redeem_metadata_warning
                raise
    ...
    async def _redeem_live_positions_unlocked(self, *, automatic: bool = False) -> None:
        if not self._wants_live_execution():
            raise RuntimeError(
                "Live execution is disabled; Direct Polymarket redeem is unavailable."
            )
        try:
            ...
            if not had_redeem_metadata_warning:
                self._add_activity(
                    "Auto-redeem checked and submitted any Direct Polymarket redeemable/claimable positions."
                    if automatic
                    else "Manual Direct Polymarket redeem/claim submitted for all resolved positions."
                )
    ...
    async def _auto_redeem_background(self) -> None:
        ...
        await self._run_redeem_claim_background(
            success_message="Auto-redeem checked and submitted any Direct Polymarket redeemable/claimable positions."
        )
    ...
    async def _force_redeem_claim_background(self) -> None:
        ...
        if not should_redeem:
            return
        await self._run_redeem_claim_background(
            success_message="Forced redeem/claim checked completed Direct Polymarket positions."
        )
    ...
    async def _run_redeem_claim_background(self, *, success_message: str) -> None:
        try:
            ...
            await self.logger.error("Direct Polymarket redeem/claim background check failed", exc)
            async with self._lock:
                self._add_activity(
                    f"Direct Polymarket redeem/claim check failed and bot kept looping: {redact_secrets(str(exc))}"
                )
        else:
            if not had_redeem_metadata_warning:
                async with self._lock:
    ...
    def _loading_balance_state(self) -> PolymarketBalanceState:
        return self.balance_state.model_copy(
            update={
                "status": "loading",
                "message": "Refreshing Direct Polymarket balance...",
            }
        )
