...
from app.domains.polymarket_direct.direct_polymarket import (
    run_direct_polymarket_json,
    run_first_direct_polymarket_json,
    utc_now,
)
from app.domains.polymarket_direct.logger import redact_secrets
...
class DirectPolymarketReadOnlyProvider:
    ...
    async def _read_leaderboard_traders(
        ...
        try:
            parsed = await run_first_direct_polymarket_json(commands)
            ...
            for trader in traders:
                trader.source_reason = (
                    f"{label.title()} profit leaderboard trader discovered via Direct Polymarket"
                )
    ...
    async def _discover_active_traders(self) -> dict[str, Any]:
        try:
            parsed = await run_direct_polymarket_json(
                [
                    "polymarket",
                    "feed",
                    "trades",
            ...
            active = [
                ...
                for trader in aggregate_traders(
                    trades,
                    "live-read",
                    "Active trader discovered from recent Direct Polymarket trade feed",
                )
    ...
    async def _read_wallet_activity(self, address: str) -> list[dict[str, Any]]:
        direct_polymarket_error: Exception | None = None
        direct_polymarket_trades: list[dict[str, Any]] = []
        try:
            parsed = await run_first_direct_polymarket_json(
                [
                    [
                        "polymarket",
                        "data",
            ...
            direct_polymarket_trades = normalize_trade_rows_for_wallet(parsed, address)
        except Exception as exc:
            direct_polymarket_error = exc
        ...
        if data_api_trades or direct_polymarket_trades:
            return dedupe_normalized_trade_rows([*data_api_trades, *direct_polymarket_trades])
        if direct_polymarket_error:
            raise direct_polymarket_error from (data_api_errors[0] if data_api_errors else None)
        if data_api_errors:
    ...
    async def _read_trending_market_trades(self) -> list[PolymarketSourceTrade]:
        try:
            markets = await run_direct_polymarket_json(
                [
                    "polymarket",
                    "markets",
                    "--active",
            ...
            self.last_discovery_status.trending_market_activity_unavailable = (
                "Trending market activity unavailable: read-only Direct Polymarket command not found. "
...
def normalize_fallback_trader_row(row: dict[str, Any]) -> PolymarketTrader | None:
    ...
    return PolymarketTrader(
        ...
        activity_source="fallback",
        direct_polymarket_profile_url=safe_http_url(
            row.get("direct_polymarketProfileUrl") or row.get("profileUrl") or row.get("url")
        ),
        polymarket_profile_url=(
            polymarket_handle_profile_url(handle)
            if handle
...
def trader_matches_leaderboard_period(
    ...
    Direct Polymarket can return high-PnL accounts for a requested daily/weekly leaderboard
    ...
    Direct Polymarket output, but reject rows that prove they are outside the requested
    leaderboard window.
    """
...
def polymarket_profile_url(address: str) -> str | None:
    return (
        f"https://polymarket-direct.com/profile/{address}"
...
def polymarket_handle_profile_url(handle: str | None) -> str | None:
    ...
    return f"https://polymarket-direct.com/@{slug}" if slug else None
...
async def run_debug_command(
    ...
    try:
        return await run_direct_polymarket_json(args)
    except Exception as exc:
