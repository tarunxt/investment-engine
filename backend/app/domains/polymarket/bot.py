...
class PolymarketPaperCopyBot:
    ...
    async def init(self) -> None:
        async with self._lock:
            ...
            await self.logger.info(
                f"Bot initialized. realMoneyTrading={self.config.live_trading} liveReads={self.config.use_live_reads}"
            )
    ...
    def _live_decision_from_source(
        ...
        reason = (
            "Detected live-market-read trade from trending market activity; eligible for Bullpen live execution."
            if source_trade.source == "live-market-read"
            else "Detected live-read trade from active trader; eligible for Bullpen live execution."
        )
        if source_trade.side == "BUY":
            amount = min(
