...
def load_polymarket_config() -> PolymarketBotConfig:
    live_unlock_mode = "manual" if os.getenv("LIVE_UNLOCK_MODE", "").strip().lower() == "manual" else "automatic"
    ...
    return PolymarketBotConfig(
        ...
        use_trending_market_activity=_bool_from_env("USE_TRENDING_MARKET_ACTIVITY", False),
        paused=_bool_from_env("PAUSED", False),
        max_pending_confirmations=_int_from_env("MAX_PENDING_CONFIRMATIONS", 10),
        max_new_live_proposals_per_poll=_int_from_env("MAX_NEW_LIVE_PROPOSALS_PER_POLL", 3),
        max_new_live_proposals_per_trader_per_poll=_int_from_env("MAX_NEW_LIVE_PROPOSALS_PER_TRADER_PER_POLL", 1),
        max_pending_per_trader=_int_from_env("MAX_PENDING_PER_TRADER", 2),
        proposal_cooldown_seconds_per_trader=_int_from_env("PROPOSAL_COOLDOWN_SECONDS_PER_TRADER", 60),
        ...
        require_manual_tracked_wallets_for_live=_bool_from_env("REQUIRE_MANUAL_TRACKED_WALLETS_FOR_LIVE", False),
        ...
        exclude_trader_handle_regex=os.getenv("EXCLUDE_TRADER_HANDLE_REGEX", "bot|shahai"),
